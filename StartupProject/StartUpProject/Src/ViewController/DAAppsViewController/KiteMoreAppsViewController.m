//
//  KiteMoreAppsViewController.m
//  KiteMoreAppsViewController
//
...
@interface KiteMoreAppsViewController () <SKStoreProductViewControllerDelegate> {
    BOOL _isLoading;
    NSString *_defaultTitle;
    BOOL boolShouldShowStatusBar;
}
...
@implementation KiteMoreAppsViewController
...
//        self.tableView.backgroundColor = DARK_BACKGROUND_COLOR;
    ...
    NSLog(@"Yes ViewDidLoad");
    
    
    [self loadAppsWithArtistId: KITEGAMESSTUDIO completionBlock:nil];
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    boolShouldShowStatusBar = [[UIApplication sharedApplication] isStatusBarHidden];
...
-(void)viewWillDisappear:(BOOL)animated {
    ...
    [[UIApplication sharedApplication] setStatusBarHidden: boolShouldShowStatusBar];
...
//        self.title = NSLocalizedString(@"Loading...",);
        ...
        self.header.text=[NSString stringWithFormat:@"Loading..."];
    } else {
        self.title = (self.pageTitle.length ? self.pageTitle : _defaultTitle);
        self.header.text=(self.pageTitle.length ? self.pageTitle : _defaultTitle);
    }
}
...
//    NSString *countryCode = [[NSLocale currentLocale] objectForKey:NSLocaleCountryCode];
    ...
    void (^returnWithResultsAndError)(NSArray *, NSError *) = ^void(NSArray *results, NSError *error) {
        ...
        NSLog(@"Returning Array %ld",(unsigned long)results.count);
        if (completion) {
...
- (void)loadAppsWithPath:(NSString *)path completionBlock:(void(^)(BOOL result, NSError *error))block{
    [self loadAppsWithPath:path defaultTitle:@"" completionBlock:block];
}
...
//                NSLog(@"Total Apps: successfull");
                ...
                if (appObject && ![mutableApps containsObject:appObject]) {
                    ...
                    if (appObject.appId != ITUNES_APPID_INT) {
            ...
            NSLog(@"Total Apps: %lu",(unsigned long)self.appsArray.count);
